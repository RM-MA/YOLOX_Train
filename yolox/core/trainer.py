...
class Trainer:
    ...
    def train_one_iter(self):
        ...
        # show_inps = inps.to("cpu")
        # show_targets = targets.to("cpu")
        # B, _, h, w = show_inps.shape
        #
        # src = np.array(show_inps, dtype=np.int8)
        # show_targets = np.array((show_targets), dtype=np.int16)
        ...
        #     cv2.line(src, tuple(np.array(target[6:8])), tuple(np.array(target[8:10])), (0, 0, 255))
        #     cv2.line(src, tuple(np.array(target[8:10])), tuple(np.array(target[0:2])), (0, 0, 255))
        # cv2.imshow("inps", src)
        # cv2.waitKey(0)

        # src = np.array(inps[0].cpu(), dtype=np.uint8)
        # src = src.transpose((1, 2, 0))
        # mask = targets[0][:, 10] > 0.3
        # tmp = targets[0][mask]
    ...
    def evaluate_and_save_model(self):
        ...
        if self.rank == 0:
            if self.args.logger == "tensorboard":
                ...
                self.tblogger.add_graph(self.model,
                                        torch.zeros(1, 3, self.input_size[1], self.input_size[0]).to(self.device))
            if self.args.logger == "wandb":
...
class TrainerWithTeacher(Trainer):
    ...
    def train_one_iter(self):
        ...
        for target in targets1:
            ...
            cv2.line(src, tuple(np.array(target[4:6])), tuple(np.array(target[6:8])), (0, 255, 0), 1)
            cv2.line(src, tuple(np.array(target[6:8])), tuple(np.array(target[8:10])), (0, 255, 0), 1)
            cv2.line(src, tuple(np.array(target[8:10])), tuple(np.array(target[0:2])), (0, 255, 0), 1)
        cv2.imshow("inps", src)
        cv2.waitKey(0)
